# Queries PubMed to find articles relating to machine learning and FUS. Returns in spreadsheet with
# basic classifications and ML and FUS related classifications. Sort of a combination between the scraper and directory parser.

# 1: fetch_pub_ids() - uses an API to first fetch a list IDs from PubMed, based on a query string
# 2: fetch_records_file() - uses an API to fetch an xml file with the full records that match the IDs from Step 1
# 3: parse_data() - parse the data, using process from 'pubcrawler_all_fields_no_deltas.py'
...
output_dir = "C:\\Users\\fuzhe\\OneDrive\\Documents\\2023 Summer Intern NLP Project 81123\\Scrape\\"
results_dir = "C:\\Users\\fuzhe\\OneDrive\\Documents\\2023 Summer Intern NLP Project 81123\\Scrape\\Results_files\\"
results_files = "C:\\Users\\fuzhe\\OneDrive\\Documents\\2023 Summer Intern NLP Project 81123\\Scrape\\Results_files\\*.xlsx"
xml_files = "C:\\Users\\fuzhe\\OneDrive\\Documents\\2023 Summer Intern NLP Project 81123\\Scrape\\xml_files\\*.xml"
xml_dir = "C:\\Users\\fuzhe\\OneDrive\\Documents\\2023 Summer Intern NLP Project 81123\\Scrape\\xml_files\\"
...
try:
    ...
    quit()
pubs_query_url = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=%28%28%22Artificial Intelligence%22[MeSH Terms]+OR+%22Machine Learning%22[MeSH Terms]+OR+%22Deep Learning%22[MeSH Terms]+OR+%22Diagnostics, Computer-Assisted%22[MeSH Terms]%29%29+AND+%28%28%22focused ultrasound%22[Title/Abstract]+OR+HIFU[Title/Abstract]+OR+MRgFUS[Title/Abstract]+OR+LIFU[Title/Abstract]+OR+%22ultrasound ablation%22[Title/Abstract]+OR+LIPU[Title/Abstract]+OR+%22ultrasound imaging%22[Title/Abstract]+OR+%22transducer%22[Title/Abstract]+OR+%22transducer%22[Other Term]+OR+%22ultrasound imaging%22[Other Term]+OR+%22focused ultrasound%22[Other Term]+OR+HIFU[Other Term]+OR+MRgFUS[Other Term]+OR+LIFU[Other Term]+OR+%22ultrasound ablation%22[Other Term]+OR+LIPU[Other Term]+OR+%22High-Intensity Focused Ultrasound Ablation%22[MeSH Terms]%29+NOT+%28%22Published Erratum%22[Publication Type]+OR+%22Retracted Publication%22[Publication Type]+OR+%22Retraction of Publication%22[Publication Type]+OR+Preprint[Publication Type]%29%29+AND+%28%28%22'+ query_start + '%22[PDAT]+:+%22' + str(current_year) + '/12/31%22[PDAT]%29%29&retmax=99999'
if len(glob.glob(results_files)) != 0:
...
def fetch_pub_ids():
    ...
    if sc == 200:
        ...
        records_data = open("C:\\Users\\fuzhe\\OneDrive\\Documents\\2023 Summer Intern NLP Project 81123\\Scrape\\pmids_from_API.txt", 'w', encoding = 'UTF-8') #delete these three lines, when running end-to-end
...
def other_info(text):
    ...
    chat = ChatOpenAI(openai_api_key=str('ENTER API KEY HERE'))

    ml_type_messages = [
        SystemMessage(content='''Please classify the article into Supervised Machine Learning, Unsupervised Machine Learning, Both, or None. Surround the answer in brackets. On a new line, write a short blurb justifying why, no longer than 5 sentences:
    ...
    treatment_cycle_messages = [
        SystemMessage(content='''Please classify the article in one of the following treatment cycles, and surround it in brackets. On a new line, write a short blurb justifying why, no longer than 5 sentences:
                       [Treatment Planning, Treatment Monitoring and Results Analysis, Patient Selection, Clinical Decision Support] Do not include brackets around your answer.'''),
        HumanMessage(content=text[0:4000])
    ]


    medical_indication_messages = [
        SystemMessage(content='''Please classify the article in one of the following medical indications, and surround it in brackets. On a new line, write a short blurb justifying why, no longer than 5 sentences.:
                       [Cardiovascular, Emerging Indications, Gynelogical, Neurological (blood-brain-barrier opening), Neurosurgery, Oncological, Urological (prostate), Veterinary, Other]. Do not include brackets around your answer.'''),
        HumanMessage(content=text[0:4000])
    ]

    key_word_messages = [
        SystemMessage(content='''Pick ONLY KEY WORDS LISTED BELOW that the article describes. Provide in a numbered list starting from 1, with no more than 10 key words.:
                       [Angular spectrum, Artificial intelligence, Artificial neural networks, Auto encoders, Bio-heat transfer, Cat Swarm Operation, Chaotic krill her algorithm (CKHA), CIVA HealthCare platform, Classification,
                        ...
                        Transformers, Ultrasonography, Ultrasound (US), U-net (CNN, Encoder, Decoder, Autoencoder), Unsupervised learning, VGG Net, Vision transformers (ViT), Wiener Filtering]. Only use the keywords in that list above'''),
        HumanMessage(content=text[0:4000])
...
# Machine Learning model for FUS/Non-Fus Classification
...
def fus_model(text):
    ...
    prediction = fus_model.predict_proba([text])

    percentage_pos = (prediction[0][0])*100
    percentage_neg = (prediction[0][1])*100
    
    # Return Focused Ultrasound related percentage followed by Non Focused Ultrasound related percentage
...
def write_to_xlsx(): # called on at end of parse_data(), if run_delta = n
    ...
    worksheet.write_string(0, 20, 'Fus/Non Fus')
    worksheet.write_string(0, 21, 'Machine Learning Type')
    worksheet.write_string(0, 22, 'Treatment Cycle')
    worksheet.write_string(0, 23, 'Medical Indications')
    worksheet.write_string(0, 24, 'Keyword(s)')
    ...
    for i in range(len(one_to_one)):
        ...
        worksheet.write_string(row_no, col + 20, fus_model(data_line[15]), format_wrap) # 20 Fus/NonFus
        worksheet.write_string(row_no, col + 21, other_info(data_line[15])[0], format_wrap) # 21 ML Type
        worksheet.write_string(row_no, col + 22, other_info(data_line[15])[1], format_wrap) # 22 Treatment Cycle
        worksheet.write_string(row_no, col + 23, other_info(data_line[15])[2], format_wrap) # 23 Medical Indications
        worksheet.write_string(row_no, col + 24, other_info(data_line[15])[3], format_wrap) # 24 Keywords
        row_no += 1
...
def write_to_xlsx_trunc():
    ...
    # API Classifications
    worksheet.write_string(0, 13, 'Fus/Non Fus')
    worksheet.write_string(0, 14, 'Machine Learning Type')
    worksheet.write_string(0, 15, 'Treatment Cycle')
    worksheet.write_string(0, 16, 'Medical Indications')
    worksheet.write_string(0, 17, 'Keyword(s)')
    ...
    for i in range(len(one_to_one)):
        ...
        worksheet.write_string(row_no, col + 20, fus_model(data_line[15]), format_wrap) # 13 Fus/NonFus
        worksheet.write_string(row_no, col + 21, other_info(data_line[15])[0], format_wrap) # 14 ML Type
        worksheet.write_string(row_no, col + 22, other_info(data_line[15])[1], format_wrap) # 15 Treatment Cycle
        worksheet.write_string(row_no, col + 23, other_info(data_line[15])[2], format_wrap) # 16 Medical Indications
        worksheet.write_string(row_no, col + 24, other_info(data_line[15])[3], format_wrap) # 17 Keywords
